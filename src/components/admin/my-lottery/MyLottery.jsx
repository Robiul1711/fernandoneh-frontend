...
const SavedNumberRow = ({ title, slug,name, logo_url, latest_numbers, special_number, index }) => {
  return (
    <motion.div 
    ...
    >
      <div className="space-y-4 w-full md:w-auto">
        <p className='font-semibold text-[20px] text-white flex items-center gap-2'> {index + 1}.  {name} </p>
      ...
      <div className="flex-shrink-0 opacity-80 group-hover:opacity-100 transition-opacity">
        <img src={logo_url} alt="Lottery Logo" className="h-10 md:h-12 w-auto object-contain" />
      </div>
    </motion.div>
  );
};
    ...
    <div className="p-4 md:p-6 space-y-8 bg-[#0D0D0D]  min-h-screen">
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'tickets' ? (
          ...
          >
            {pinnedLotteries?.data?.map((ticket, i) => (
